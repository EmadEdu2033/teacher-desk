// English + Arabic translations for Teacher Desk

const dict = {
  en: {
    appName: "Teacher Desk",
    "tab.notes": "Sticky Notes",
    "tab.tasks": "Tasks",
    "tab.settings": "Settings",
    "podium.off": "Podium: Off",
    "podium.on":  "Podium: On",
    "notes.new": "+ New Note",
    "notes.pickColor": "Pick a color for your new note",
    "notes.delete": "Delete",
    "notes.duplicate": "Duplicate",
    "notes.bringFront": "Bring to front",
    "notes.changeColor": "Change color",
    "notes.titlePlaceholder": "Note title",
    "notes.bodyPlaceholder": "Write here…",
    "notes.fontSize": "Font size",
    "notes.fontSmall": "Small",
    "notes.fontMedium": "Medium",
    "notes.fontLarge": "Large",
    "notes.confirmDelete": "Delete this note?",
    "wall.fit": "Fit",
    "wall.reset": "Reset",
    "wall.empty": "Click \"+ New Note\" to add your first note",
    "tasks.new": "+ New Task",
    "tasks.search": "Search tasks…",
    "tasks.allPriorities": "All priorities",
    "tasks.allCategories": "All categories",
    "tasks.confirmDelete": "Delete this task?",
    "tasks.subtaskPlaceholder": "Sub-task",
    "tasks.urgent": "Urgent",
    "tasks.high": "High",
    "tasks.normal": "Normal",
    "tasks.low": "Low",
    "tasks.title": "Title",
    "tasks.description": "Description",
    "tasks.priority": "Priority",
    "tasks.category": "Category",
    "tasks.dueDate": "Due date",
    "tasks.dueTime": "Due time",
    "tasks.subtasks": "Sub-tasks",
    "tasks.addSubtask": "+ Add sub-task",
    "tasks.today": "Today",
    "tasks.upcoming": "Upcoming",
    "tasks.overdue": "Overdue",
    "tasks.noDate": "No date",
    "tasks.completed": "Completed",
    "tasks.empty": "No tasks yet — click \"+ New Task\".",
    "tasks.dueLabel": "Due",
    "tasks.reminderTitle": "Task due",
    "common.cancel": "Cancel",
    "common.save": "Save",
    "common.edit": "Edit",
    "common.delete": "Delete",
    "settings.title": "Settings",
    "settings.language": "Language",
    "settings.theme": "Theme",
    "settings.themeLight": "Light",
    "settings.themeDark": "Dark",
    "settings.themeSystem": "System",
    "settings.podium": "Podium mode (hide from screen capture)",
    "settings.podiumHelp": "When ON, Zoom / Google Meet / OBS show a black box where Teacher Desk is. You still see it normally.",
    "settings.sound": "Reminder sound",
    "settings.data": "Your Data",
    "settings.export": "Export backup…",
    "settings.import": "Import backup…",
    "settings.openFolder": "Open data folder",
    "settings.dataHelp": "All your notes & tasks are stored only on this computer.",
    "settings.autoBackup": "Automatic Backups",
    "settings.autoBackupHelp": "Teacher Desk automatically saves a snapshot of your notes & tasks each day. The last 7 are kept.",
    "settings.autoBackupFolder": "Backups folder",
    "settings.autoBackupEmpty": "No automatic backups yet — one will be created the next time you open Teacher Desk.",
    "settings.autoBackupWebOnly": "Automatic backups are only available in the installed Windows app.",
    "settings.backupStatusLast": "Last backup: {when}.",
    "settings.backupStatusWarn": "It has been more than 2 days since the last backup. Open Teacher Desk regularly so a fresh snapshot is saved.",
    "settings.backupFailedTitle": "Last automatic backup failed",
    "settings.backupFailedDetail": "Tried {when}. Reason: {reason}",
    "settings.backupFailedHint": "Your previous snapshots are still safe. Common fixes: free up disk space, allow Teacher Desk in your antivirus, or check the backups folder permissions.",
    "settings.backupFailedUnknownReason": "Unknown error.",
    "settings.backupTimeToday": "today at {time}",
    "settings.backupTimeYesterday": "yesterday at {time}",
    "settings.backupTimeDaysAgo": "{n} days ago at {time}",
    "settings.backupTimeOn": "{date} at {time}",
    "settings.restoreAuto": "Restore from backup…",
    "settings.restore": "Restore",
    "settings.backupNow": "Back up now",
    "settings.about": "About",
    "about.coachName": "Coach Emad",
    "about.creator": "Created by Coach Emad",
    "about.version": "Teacher Desk v{version}",
    "about.versionLoading": "Teacher Desk —",
    "credit.line": "Created by Coach Emad",
    "theme.toggle": "Toggle dark mode",
    "lang.toggleToAr": "التبديل إلى العربية",
    "lang.toggleToEn": "Switch to English",
    "lang.shortAr": "ع",
    "lang.shortEn": "EN",
    "toast.exported": "Backup saved.",
    "toast.imported": "Backup restored.",
    "toast.backupNowSuccess": "Backup created.",
    "toast.backupNowFailed": "Couldn't create a backup. Please try again.",
    "toast.error": "Something went wrong.",
    "toast.podiumOn": "Podium mode ON — hidden from screen capture.",
    "toast.podiumOff": "Podium mode OFF — visible to screen capture.",
    "settings.confirmImport": "Importing will replace your current data. Continue?",
  },
  ar: {
    appName: "مكتب المعلم",
    "tab.notes": "ملاحظات لاصقة",
    "tab.tasks": "المهام",
    "tab.settings": "الإعدادات",
    "podium.off": "وضع المنصة: مغلق",
    "podium.on":  "وضع المنصة: مفعّل",
    "notes.new": "+ ملاحظة جديدة",
    "notes.pickColor": "اختر لون الملاحظة الجديدة",
    "notes.delete": "حذف",
    "notes.duplicate": "تكرار",
    "notes.bringFront": "إحضار للأمام",
    "notes.changeColor": "تغيير اللون",
    "notes.titlePlaceholder": "عنوان الملاحظة",
    "notes.bodyPlaceholder": "اكتب هنا…",
    "notes.fontSize": "حجم الخط",
    "notes.fontSmall": "صغير",
    "notes.fontMedium": "متوسط",
    "notes.fontLarge": "كبير",
    "notes.confirmDelete": "حذف هذه الملاحظة؟",
    "wall.fit": "ملاءمة",
    "wall.reset": "إعادة",
    "wall.empty": "اضغط «+ ملاحظة جديدة» لإضافة أول ملاحظة",
    "tasks.new": "+ مهمة جديدة",
    "tasks.search": "ابحث في المهام…",
    "tasks.allPriorities": "كل الأولويات",
    "tasks.allCategories": "كل التصنيفات",
    "tasks.confirmDelete": "حذف هذه المهمة؟",
    "tasks.subtaskPlaceholder": "مهمة فرعية",
    "tasks.urgent": "عاجلة",
    "tasks.high": "عالية",
    "tasks.normal": "عادية",
    "tasks.low": "منخفضة",
    "tasks.title": "العنوان",
    "tasks.description": "الوصف",
    "tasks.priority": "الأولوية",
    "tasks.category": "التصنيف",
    "tasks.dueDate": "تاريخ الاستحقاق",
    "tasks.dueTime": "وقت الاستحقاق",
    "tasks.subtasks": "مهام فرعية",
    "tasks.addSubtask": "+ إضافة مهمة فرعية",
    "tasks.today": "اليوم",
    "tasks.upcoming": "القادمة",
    "tasks.overdue": "متأخرة",
    "tasks.noDate": "بلا تاريخ",
    "tasks.completed": "مكتملة",
    "tasks.empty": "لا توجد مهام بعد — اضغط «+ مهمة جديدة».",
    "tasks.dueLabel": "موعد",
    "tasks.reminderTitle": "حان موعد المهمة",
    "common.cancel": "إلغاء",
    "common.save": "حفظ",
    "common.edit": "تعديل",
    "common.delete": "حذف",
    "settings.title": "الإعدادات",
    "settings.language": "اللغة",
    "settings.theme": "المظهر",
    "settings.themeLight": "فاتح",
    "settings.themeDark": "داكن",
    "settings.themeSystem": "النظام",
    "settings.podium": "وضع المنصة (إخفاء من تسجيل الشاشة)",
    "settings.podiumHelp": "عند التفعيل، تظهر Zoom / Google Meet / OBS مربعاً أسود مكان مكتب المعلم. أنت تراه طبيعياً.",
    "settings.sound": "صوت التذكير",
    "settings.data": "بياناتك",
    "settings.export": "حفظ نسخة احتياطية…",
    "settings.import": "استيراد نسخة احتياطية…",
    "settings.openFolder": "فتح مجلد البيانات",
    "settings.dataHelp": "كل ملاحظاتك ومهامك مخزنة على هذا الجهاز فقط.",
    "settings.autoBackup": "النسخ الاحتياطي التلقائي",
    "settings.autoBackupHelp": "يحفظ مكتب المعلم نسخة من ملاحظاتك ومهامك كل يوم تلقائياً. يتم الاحتفاظ بآخر 7 نسخ.",
    "settings.autoBackupFolder": "مجلد النسخ الاحتياطي",
    "settings.autoBackupEmpty": "لا توجد نسخ احتياطية بعد — ستُنشأ نسخة عند فتح مكتب المعلم في المرة القادمة.",
    "settings.autoBackupWebOnly": "النسخ الاحتياطي التلقائي متاح فقط في تطبيق ويندوز المثبَّت.",
    "settings.backupStatusLast": "آخر نسخة احتياطية: {when}.",
    "settings.backupStatusWarn": "مرّ أكثر من يومين على آخر نسخة احتياطية. افتح مكتب المعلم بانتظام لحفظ نسخة جديدة.",
    "settings.backupFailedTitle": "فشل آخر نسخة احتياطية تلقائية",
    "settings.backupFailedDetail": "حدث ذلك {when}. السبب: {reason}",
    "settings.backupFailedHint": "نسخك السابقة لا تزال آمنة. حلول شائعة: أفرغ مساحة على القرص، اسمح لمكتب المعلم في برنامج الحماية، أو تحقق من صلاحيات مجلد النسخ الاحتياطي.",
    "settings.backupFailedUnknownReason": "خطأ غير معروف.",
    "settings.backupTimeToday": "اليوم في {time}",
    "settings.backupTimeYesterday": "أمس في {time}",
    "settings.backupTimeDaysAgo": "قبل {n} يوماً في {time}",
    "settings.backupTimeOn": "{date} في {time}",
    "settings.restoreAuto": "استرجاع من نسخة احتياطية…",
    "settings.restore": "استرجاع",
    "settings.backupNow": "إنشاء نسخة احتياطية الآن",
    "settings.about": "عن البرنامج",
    "about.coachName": "Coach Emad",
    "about.creator": "مقدم من Coach Emad",
    "about.version": "مكتب المعلم — الإصدار {version}",
    "about.versionLoading": "مكتب المعلم —",
    "credit.line": "مقدم من كوتش عماد",
    "theme.toggle": "تبديل الوضع الداكن",
    "lang.toggleToAr": "التبديل إلى العربية",
    "lang.toggleToEn": "Switch to English",
    "lang.shortAr": "ع",
    "lang.shortEn": "EN",
    "toast.exported": "تم حفظ النسخة الاحتياطية.",
    "toast.imported": "تم استرجاع النسخة الاحتياطية.",
    "toast.backupNowSuccess": "تم إنشاء نسخة احتياطية.",
    "toast.backupNowFailed": "تعذّر إنشاء النسخة الاحتياطية. يرجى المحاولة مرة أخرى.",
    "toast.error": "حدث خطأ ما.",
    "toast.podiumOn": "وضع المنصة مفعّل — مخفي عن تسجيل الشاشة.",
    "toast.podiumOff": "وضع المنصة مغلق — ظاهر في تسجيل الشاشة.",
    "settings.confirmImport": "سيؤدي الاستيراد إلى استبدال بياناتك الحالية. هل تريد المتابعة؟",
  },
};

let currentLang = 'en';

export function setLang(lang) {
  currentLang = (lang === 'ar') ? 'ar' : 'en';
  document.documentElement.setAttribute('lang', currentLang);
  document.documentElement.setAttribute('dir', currentLang === 'ar' ? 'rtl' : 'ltr');
  applyDom();
}

export function t(key) {
  return (dict[currentLang] && dict[currentLang][key]) || (dict.en[key]) || key;
}

export function getLang() { return currentLang; }

export function applyDom() {
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const k = el.getAttribute('data-i18n');
    el.textContent = t(k);
  });
  document.querySelectorAll('[data-i18n-attr]').forEach(el => {
    const spec = el.getAttribute('data-i18n-attr');
    spec.split(';').forEach(part => {
      const [attr, key] = part.split(':');
      if (attr && key) el.setAttribute(attr.trim(), t(key.trim()));
    });
  });
}
